    ...
    <div class="tab-content">
        <div class="tab-pane active" id="signin">
            ...
            <div style="text-align: center;font-size: 22px;">
                <c:choose>
                    <c:when test="${parentWasAdded}">
                        <span>שלום <c:out value="${firstName}" />, ההצטרפות לקבוצה <strong><c:out value="${groupName}" /></strong> בוצעה בהצלחה.</span>
                    </c:when>
                    <c:otherwise>
                        <span>שלום <c:out value="${firstName}" />, הינך כבר ${friendWithGender} בקבוצה <strong><c:out value="${groupName}" /></strong>.</span>
                    </c:otherwise>
                </c:choose>
            </div>

            <c:choose>
                <c:when test="${unusedInvitations.size() > 0}">
                    <hr>
                    <div class="invitation-management">
                        <form method="post" action="/parent/acceptInvitations">
                            <c:choose>
                                <c:when test="${unusedInvitations.size() == 1}">
                                    <div><span class="single-invitation">הזמנה אחת   לקבוצה <strong>${unusedInvitations.get(0).parentsGroup.name}</strong> שנשלחה ע''י <strong>${unusedInvitations.get(0).inviter.displayName}</strong> ממתינה לך</span></div>
                                    <input type="hidden" name="invitationsIds" checked="checked" value="${unusedInvitations.get(0).id}">
                                    <br><div><input class="btn btn-primary pull-left join-group-btn" type="submit" value="הצטרפות"/></div>
                                </c:when>
                                <c:otherwise>
                                    <div style="padding-bottom: 10px;">
                                        <span class="multi-invitation-title">${unusedInvitations.size()} הזמנות לקבוצות ממתינות לך באתר</span>
                                    </div>
            ...
            <hr>
            <p><a href="${skipUrl}">המשך לאתר</a></p>
        </div><!-- /.tab-pane -->
