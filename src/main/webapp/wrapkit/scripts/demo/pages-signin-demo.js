function loginUser() {
	...
	var originalBtnHtml = $btnSignin.html();
	$btnSignin.html( '<i class="fa fa-spin fa-spinner"></i> טוען...' );//todo i18n 'loading'
	...
	posting.done(function(response) {
		...
		if (response.error) {
			$btnSignin.html(originalBtnHtml);
			$('#login-error-msg').html(response.errorMessage);
			$('#login-error-panel').slideDown();
		} else {
			window.location.replace(response.responseObject);
		}
	});
}
...
function signupUser() {
	...
	var originalBtnHtml = $btnSignup.html();
	$btnSignup.html( '<i class="fa fa-spin fa-spinner"></i> טוען...' );//todo i18n 'loading'
	...
	posting.done(function(response) {
		...
		if (response.error) {
			$btnSignup.html(originalBtnHtml);
			$('#signup-error-msg').html(response.errorMessage);
			$('#signup-error-panel').slideDown();
		} else {
			eraseCookie('PARENT_SAW_WELCOME_MSG_COOKIE');
			window.location.replace(response.responseObject);
		}
	});
}
